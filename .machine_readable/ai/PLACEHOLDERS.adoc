...
sed -i 's/Session Sentinel/my-project/g' $(grep -rl 'Session Sentinel' .)
sed -i 's/SESSION_SENTINEL/MY_PROJECT/g' $(grep -rl 'SESSION_SENTINEL' .)
sed -i 's/session_sentinel/my_project/g' $(grep -rl 'session_sentinel' .)
sed -i 's/session-sentinel/my-project/g' $(grep -rl 'session-sentinel' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Session Sentinel` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `{{PROJECT_DESCRIPTION}}` | One-line description | `A tool for X` | flake.nix |
| `SESSION_SENTINEL` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `session_sentinel` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `session-sentinel` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `https://github.com/hyperpolymath/session-sentinel` | Project website | `https://example.org` | SECURITY.md |
...
| `Multi-AI session health monitor with system tray, self-healing, and PanLL panel` | One-line project description | `FFI bridges between languages` | STATE.a2ml |

### AI Manifest

| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Session Sentinel` | Repository name | `my-project` | 0-AI-MANIFEST.a2ml |
