...
init:
    ...
    # Also replace Session Sentinel and hyperpolymath in AI manifest
...
build *args:
    @echo "Building session_sentinel (debug)..."
    cd src/interface/ffi && zig build {{args}}
    @echo "Build complete"

# Build in release mode with optimizations
build-release *args:
    @echo "Building session_sentinel (release)..."
    cd src/interface/ffi && zig build -Doptimize=ReleaseFast {{args}}
    @echo "Release build complete"

# Type-check AffineScript core logic
check-as:
    @echo "Type-checking AffineScript core..."
    @for f in src/core/*.as; do affinescript check "$$f" && echo "  OK: $$f" || echo "  FAIL: $$f"; done

# Type-check Idris2 ABI definitions
check-abi:
    @echo "Type-checking Idris2 ABI..."
    cd src/interface/abi && idris2 --check HealthState.idr MonitorProtocol.idr PanelProtocol.idr

# Compile AffineScript to WASM
compile-as:
    @echo "Compiling AffineScript to WASM..."
    @mkdir -p build/wasm
    @for f in src/core/*.as; do affinescript compile "$$f" -o "build/wasm/$$(basename $$f .as).wasm" && echo "  OK: $$f"; done

# Build and watch for changes
build-watch:
    @echo "Watching for changes..."
    find src -name '*.zig' -o -name '*.as' -o -name '*.idr' | entr -c just build
...
# Run the application (single scan)
run *args: build
    ./src/interface/ffi/zig-out/bin/session-sentinel-tray --once {{args}}

# Run as daemon with verbose output
run-verbose *args: build
    ./src/interface/ffi/zig-out/bin/session-sentinel-tray --daemon --verbose {{args}}

# Install binary, config, and systemd service
install: build-release
    @echo "Installing session_sentinel..."
    cp src/interface/ffi/zig-out/bin/session-sentinel-tray ~/.local/bin/
    chmod +x systemd/install.sh && ./systemd/install.sh
    @echo "Installed! Start with: systemctl --user start session-sentinel.service"
...
deps:
    ...
    @command -v idris2 >/dev/null && echo "  idris2: $(idris2 --version 2>&1 | head -1)" || echo "  idris2: MISSING (optional — for ABI verification)"
    @command -v affinescript >/dev/null && echo "  affinescript: available" || echo "  affinescript: MISSING (optional — for core type-checking)"
    @pkg-config --exists dbus-1 && echo "  libdbus-1: $(pkg-config --modversion dbus-1)" || echo "  libdbus-1: MISSING (required)"
...
cookbook:
    ...
    echo "= session_sentinel Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/session_sentinel.1 << EOF
    .TH session_sentinel 1 "$(date +%Y-%m-%d)" "{{version}}" "session_sentinel Manual"
    .SH NAME
    session_sentinel \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/session_sentinel.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [session_sentinel]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-session_sentinel}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t session_sentinel:latest -f container/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t session_sentinel:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push session_sentinel:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it session_sentinel:latest {{args}}
