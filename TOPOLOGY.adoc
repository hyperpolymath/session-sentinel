...
Multi-AI session health monitor with system tray, self-healing, and
PanLL panel. Watches storage consumption across multiple AI tools
(Claude, Copilot, Ollama, LM Studio, Continue.dev, Cursor, Aider, custom
providers), provides real-time health status via KDE system tray,
automatically cleans up stale sessions, and exposes fine-grained
analysis/control via PanLL panel.
