...
# Session Sentinel container entrypoint
#
...
cleanup() {
    echo "Received shutdown signal — stopping session-sentinel..."
...
echo "Starting session-sentinel..."
echo "  Host: ${APP_HOST:-[::]}"
echo "  Port: ${APP_PORT:-9847}"
...
# Examples:
#   exec /app/session-sentinel
#   exec /app/release/bin/session-sentinel start
#   exec /app/session-sentinel serve --host "${APP_HOST}" --port "${APP_PORT}"
