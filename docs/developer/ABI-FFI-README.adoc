...
# SESSION_SENTINEL ABI/FFI Documentation
...
│  generated/abi/session_sentinel.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsession_sentinel.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── session_sentinel.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── session_sentinel.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/session_sentinel.h
```
...
```c
#include "session_sentinel.h"

int main() {
    void* handle = session_sentinel_init();
    if (!handle) return 1;

    int result = session_sentinel_process(handle, 42);
    if (result != 0) {
        const char* err = session_sentinel_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    session_sentinel_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lsession_sentinel -L./zig-out/lib
```

### From Idris2

```idris
import SESSION_SENTINEL.ABI.Foreign
...
```rust
#[link(name = "session_sentinel")]
extern "C" {
    fn session_sentinel_init() -> *mut std::ffi::c_void;
    fn session_sentinel_free(handle: *mut std::ffi::c_void);
    fn session_sentinel_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = session_sentinel_init();
        assert!(!handle.is_null());

        let result = session_sentinel_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsession_sentinel = "libsession_sentinel"

function init()
    handle = ccall((:session_sentinel_init, libsession_sentinel), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:session_sentinel_process, libsession_sentinel), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:session_sentinel_free, libsession_sentinel), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/session_sentinel.h
   ```
